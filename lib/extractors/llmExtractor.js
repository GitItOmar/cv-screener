...
export class LLMExtractor {
  ...
  async extractResumeData(resumeText, options = {}) {
    console.info('[LLMExtractor] Starting resume data extraction');
    console.debug('[LLMExtractor] Input text length:', resumeText.length);
    ...
    try {
      ...
      console.debug('[LLMExtractor] Prepared text length:', preparedText.length);
      ...
      for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        try {
          console.debug(`[LLMExtractor] Extraction attempt ${attempt}/${maxRetries + 1}`);

          extractedData = await this.performExtraction(preparedText, config, attempt);

          if (extractedData) {
            console.info('[LLMExtractor] Extraction successful on attempt', attempt);
            break;
          }
        } catch (error) {
          lastError = error;
          console.warn(`[LLMExtractor] Attempt ${attempt} failed:`, error.message);
          ...
          console.debug(`[LLMExtractor] Waiting ${waitTime}ms before retry`);
      ...
      if (config.validateResults) {
        extractedData = await this.validateAndCleanResults(extractedData, preparedText);
      }
      ...
      console.info('[LLMExtractor] Resume extraction completed successfully');
      console.debug('[LLMExtractor] Total processing time:', processingTime, 'ms');

      return extractedData;
    } catch (error) {
      const processingTime = Date.now() - startTime;
      console.error('[LLMExtractor] Extraction failed after', processingTime, 'ms:', error.message);
  ...
  async performExtraction(preparedText, config, attempt) {
    console.debug('[LLMExtractor] Performing LLM extraction');

    // Build the appropriate prompt
    let messages;
    if (config.useEnhancedPrompts && attempt === 1) {
      console.debug('[LLMExtractor] Using enhanced prompts with examples');
      messages = PromptTemplates.getEnhancedPrompt(preparedText);
    } else {
      console.debug('[LLMExtractor] Using minimal prompts');
   ...
   */
  async validateAndCleanResults(extractedData, originalText) {
    console.debug('[LLMExtractor] Validating extraction results');
    ...
    try {
      ...
      console.debug('[LLMExtractor] Validation completed successfully');
      return cleanedData;
    } catch (error) {
      console.warn('[LLMExtractor] Validation failed:', error.message);

      // Try to fix common issues
      const fixedData = this.attemptDataRepair(extractedData);

      if (fixedData) {
        console.info('[LLMExtractor] Data repair successful');
        return fixedData;
      }
  ...
  cleanExtractedData(data) {
    console.debug('[LLMExtractor] Cleaning extracted data');
    ...
    if (cleaned.basicInformation) {
      ...
      if (cleaned.basicInformation.phone) {
        cleaned.basicInformation.phone = cleaned.basicInformation.phone.replace(
          /[^\d+\-() ]/g,
          '',
        );
      }
  ...
  validateDataConsistency(data) {
    console.debug('[LLMExtractor] Validating data consistency');
  ...
  attemptDataRepair(data) {
    console.debug('[LLMExtractor] Attempting data repair');
    ...
    try {
      ...
      console.error('[LLMExtractor] Data repair failed:', error.message);
      return null;
    }
  }
  ...
  updateOptions(newOptions) {
    ...
    console.info('[LLMExtractor] Options updated:', newOptions);
  }
}
