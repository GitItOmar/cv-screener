...
export class ResumeAgent {
  ...
  initializeClient() {
    ...
    // Support custom base URL for Azure OpenAI or other providers
    if (process.env.OPENAI_BASE_URL) {
      config.baseURL = process.env.OPENAI_BASE_URL;
    }

    this.client = new OpenAI(config);
    console.info('[ResumeAgent] OpenAI client initialized');
  ...
  async extractResumeData(resumeText, options = {}) {
    console.info('[ResumeAgent] Starting resume data extraction');
    console.debug('[ResumeAgent] Text length:', resumeText.length);
    ...
    try {
      ...
      console.debug('[ResumeAgent] Sending request to OpenAI');
      ...
      console.info('[ResumeAgent] Extraction completed in', processingTime, 'ms');
      ...
      const processingTime = Date.now() - startTime;
      console.error('[ResumeAgent] Extraction failed after', processingTime, 'ms:', error.message);
   ...
   */
  buildExtractionPrompt(resumeText, options) {
...
Return only the JSON object with the extracted information.`;
  ...
  async makeAPIRequest(messages, options) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.debug(`[ResumeAgent] API attempt ${attempt}/${maxRetries}`);
        ...
        console.debug('[ResumeAgent] API request successful');
        return response;
      } catch (error) {
        lastError = error;
        console.warn(`[ResumeAgent] API attempt ${attempt} failed:`, error.message);
        ...
        console.debug(`[ResumeAgent] Waiting ${waitTime}ms before retry`);
  ...
  parseAPIResponse(response) {
    console.debug('[ResumeAgent] Parsing API response');
    ...
    try {
      ...
      console.debug('[ResumeAgent] Successfully parsed JSON response');
      ...
      console.error('[ResumeAgent] Failed to parse API response:', error.message);
      console.debug('[ResumeAgent] Raw response:', choice.message.content);
  ...
  validateResponseStructure(data) {
    ...
    for (const section of requiredSections) {
      if (!(section in data)) {
        console.warn(`[ResumeAgent] Missing required section: ${section}`);
    ...
    console.debug('[ResumeAgent] Response structure validation completed');
  }
  ...
  async handleRateLimit() {
    ...
    if (timeSinceLastRequest < minInterval) {
      ...
      console.debug(`[ResumeAgent] Rate limiting: waiting ${waitTime}ms`);
      await new Promise((resolve) => setTimeout(resolve, waitTime));
    }
  ...
  updateCostTracking(usage) {
    ...
    console.debug('[ResumeAgent] Cost tracking updated:', {
      requests: this.costTracker.requestCount,
      totalTokens: this.costTracker.totalTokensUsed,
  ...
  resetCostTracking() {
    ...
    console.info('[ResumeAgent] Cost tracking reset');
  }
  ...
  async testConnection() {
    try {
      ...
      console.info('[ResumeAgent] Connection test successful');
      return true;
    } catch (error) {
      console.error('[ResumeAgent] Connection test failed:', error.message);
      return false;
    }
  }
}
