...
export class CSVParser {
  ...
  static async parse(buffer) {
    console.debug('[CSVParser] Starting CSV parsing');
    ...
    try {
      ...
      console.debug('[CSVParser] CSV string length:', csvString.length);
      ...
      console.info('[CSVParser] Successfully parsed', records.length, 'records');

      // Convert records to formatted text
      const formattedText = this.formatCSVAsText(records);

      console.debug('[CSVParser] Formatted text length:', formattedText.length);
      return formattedText;
    } catch (error) {
      console.error('[CSVParser] Failed to parse CSV:', error.message);
  ...
  static parseCSVString(csvString) {
    ...
    for (const delimiter of delimiters) {
      try {
        console.debug('[CSVParser] Trying delimiter:', delimiter === '\t' ? 'TAB' : delimiter);
        ...
        console.debug('[CSVParser] Delimiter', delimiter, 'failed:', error.message);
        continue;
      }
    }
    ...
    console.info('[CSVParser] Best result: delimiter with', maxColumns, 'columns');
    return bestResult;
  }
  ...
  static formatCSVAsText(records) {
    console.debug('[CSVParser] Formatting', records.length, 'records as text');
    ...
    console.debug('[CSVParser] Formatting completed');
    return result;
  }
  ...
  static async getStatistics(buffer) {
    try {
      ...
      console.warn('[CSVParser] Failed to get statistics:', error.message);
      return {};
    }
  }
}
